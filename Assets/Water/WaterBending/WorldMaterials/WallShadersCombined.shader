    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct v2f
            {
                ...
                float3 worldNormal : TEXCOORD2; // Normal original, n�o deformada
            ...
            v2f vert(appdata v)
            {
                ...
                float3 modifiedWorldPos = worldVertex.xyz; // Come�a com a posi��o original
                ...
                if (_BulletType == 0.0)
                {
                    for (int i = 0; i < 512; i++)
                    {
                        ...
                        if (ponto.w > 0.001) // Se o impacto est� ativo (tempo de impacto v�lido)
                        {
                            ...
                            // Por simplicidade, vamos faz�-lo persistir enquanto age < _EmissionDecayTime * 1.5 (um pouco mais que o glow forte)
                            // Ou, para que o amassado seja t�o persistente quanto o brilho m�nimo,
                            // ele s� desaparece quando o ponto � sobrescrito no array.
                            // A condi��o ponto.w > 0.001 j� garante isso.
                            ...
                            if (dist <= _Radius)
                            {
                                ...
                                t = t * t; // Para um falloff mais acentuado (quadr�tico)

                                // Empurra o v�rtice para dentro ao longo da sua normal original
                                // O ponto.w (tempo) n�o � usado para a for�a aqui, apenas para saber se est� ativo.
                ...
                else if (_BulletType == 2.0)
                {
                    for (int i = 0; i < 512; i++)
                    {
                        ...
                        if (ponto.w > 0.0) // ponto.w � a intensidade
                        {
                            float3 delta = modifiedWorldPos - ponto.xyz;
                            float dist = length(delta);
                            ...
                            if (dist <= _Radius)
                            {
                                ...
                                // Empurr�o ao longo da normal
                                modifiedWorldPos -= worldNormalInput * _ImpactPushStrength * t * ponto.w; // renomeei _Impact para _ImpactPushStrength antes, mas wind usava _Impact. Usando _ImpactPushStrength aqui tamb�m.
                ...
                worldVertex.xyz = modifiedWorldPos; // Aplica a modifica��o final

                o.vertex = UnityWorldToClipPos(worldVertex);
                o.uv = TRANSFORM_TEX(v.uv, _MainTex);
                o.worldPos = worldVertex.xyz; // Posi��o mundial potencialmente modificada
                ...
                if (_BulletType == 0.0)
                {
                    for (int k = 0; k < 512; k++)
                    {
                        ...
                        if (ponto.w > 0.001) 
                        {
                            ...
                            if (age >= 0) // Impacto aconteceu ou est� acontecendo
                            {
                                if (age < _EmissionDecayTime)
                                {
                                    // Interpola do m�ximo para o m�nimo durante o tempo de decaimento
                                ...
                                {
                                    // Ap�s o tempo de decaimento, mant�m o brilho m�nimo
                    ...
                    // A cor base N�O � modificada aqui, apenas a emiss�o � adicionada.
                    // Se voc� quiser que a textura de impacto substitua a _MainTex na �rea afetada:
                    // float totalImpactMaskForAlbedo = 0;
                    // (calcular totalImpactMaskForAlbedo de forma similar a accumulatedEmission, mas sem strength, s� a m�scara)
                    // baseColor.rgb = lerp(baseColor.rgb, tex2D(_ImpactTex, i.uv).rgb * _ImpactEmissionColor.rgb, saturate(totalImpactMaskForAlbedo));
                    // Mas para um "glow" em cima, adicionar � emiss�o � o correto.
                }
                // Efeito de Polui��o/�gua (BulletType == 1.0)
