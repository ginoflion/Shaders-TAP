...
public class BallTrailPainter : MonoBehaviour
{
    ...
    void Start()
    {
        // ... (same Start method as before) ...
        if (ballTransform == null) { Debug.LogError("BallTrailPainter: Ball Transform not assigned!", this); enabled = false; return; }
        if (floorMaterial == null) { Debug.LogError("BallTrailPainter: Floor Material not assigned!", this); enabled = false; return; }
        if (floorTransform == null) { Debug.LogError("BallTrailPainter: Floor Transform not assigned!", this); enabled = false; return; }
        ...
        _lastKnownBallPositionWS = ballTransform != null ? ballTransform.position + Vector3.down * 10000 : Vector3.down * 10000; // Initialize far away
        floorMaterial.SetVector(BallPositionWSID, _lastKnownBallPositionWS);
        floorMaterial.SetVector(TrailOriginWSID, _lastKnownBallPositionWS); // Also set origin far away initially
    }
